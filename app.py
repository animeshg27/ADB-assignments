...
@app.route('/popupdate',methods=['POST','GET'])
def update():
    if (request.method=='POST'):
        State= str(request.form['State'])
        Increment= str(request.form['Increment'])
        querry="UPDATE name SET Population = "+Increment+" WHERE Name ="+State+" "
        cursor.execute(querry)
        querry2="Select * from name WHERE Name =  "+State+" "
        cursor.execute(querry2)
        results = cursor.fetchall()
    return render_template("list.html",rows = results)


@app.route('/Citydelete', methods=['GET', 'POST'])
def deleterecord():
    if (request.method=='POST'):
        State= str(request.form['State'])
        querry="DELETE FROM name WHERE State ="+State+"' "
        cursor.execute(querry)
